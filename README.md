...
6. **Backend env (required):** The API refuses to start unless `AUTH0_ISSUER_BASE_URL` (or `AUTH0_DOMAIN`), `AUTH0_CLIENT_ID`, and `AUTH0_CLIENT_SECRET` are set in `backend/.env`. In Auth0, set **Allowed Callback URLs** to include your Flask origin, e.g. `http://localhost:5001/api/auth/callback`, plus your production API URL when deployed.
